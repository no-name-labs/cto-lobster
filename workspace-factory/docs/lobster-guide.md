...
**Lobster-first architecture**: For agent creation and editing, lobster owns the ENTIRE execution pipeline. CTO does intake and writes prompt files (RESEARCH.txt, T1-T6.txt, SMOKE.txt, VERIFY.txt), then launches lobster via `exec`. Lobster handles research, build (via codex with `--sandbox danger-full-access`), test, register, gateway restart, smoke, verify, approval gate, and apply. CTO does NOT run codex directly for agent creation.
...
**Always Lobster:**
- Agent creation: research → build (codex) → test → register → gateway restart → smoke → verify → approval → apply
- Agent editing: similar pipeline with edit-specific steps
- System diagnostics: detect → diagnose → report

**Never Lobster (LLM reasoning required):**
- INTAKE conversation
- REQUIREMENTS_SIGNOFF (user dialogue)
- Prompt file writing (RESEARCH.txt, T1-T6.txt, SMOKE.txt, VERIFY.txt)
- Non-creation code tasks (factory-codegen) — CTO delegates to codex/claude directly
...
Pipelines in `workspace-factory/lobster/`:

| File | Purpose | Entry point |
|---|---|---|
| `create-agent.lobster` | Full agent creation: research → build (codex) → test → register → gateway restart → smoke → verify → approval gate → apply | After CTO writes prompt files (RESEARCH.txt, T1-T6.txt, SMOKE.txt, VERIFY.txt) |
| `edit-agent.lobster` | Full agent editing: similar pipeline with edit-specific steps | After CTO writes prompt files |
| `system-diagnostic.lobster` | System diagnostics: detect → diagnose → report | On diagnostic request |

**These are the only lobster files that exist.** No other pipelines (cron-manage, gateway-restart, etc.).

## Using Lobster in the CTO state machine

### Agent creation/editing (lobster-first path)

CTO does intake and writes prompt files, then hands off entirely to lobster:

```
INTAKE
  → PROMPT_WRITING (CTO writes RESEARCH.txt, T1-T6.txt, SMOKE.txt, VERIFY.txt)
  → LOBSTER_EXEC: exec create-agent.lobster (or edit-agent.lobster)
      lobster: research (RESEARCH.txt)
      lobster: build via codex (T1-T6.txt, --sandbox danger-full-access)
      lobster: test
      lobster: register agent in openclaw.json
      lobster: gateway restart
      lobster: smoke (SMOKE.txt)
      lobster: verify (VERIFY.txt)
      lobster: approval_gate  ← pauses here (READY_FOR_APPLY)
      resume(approve=true)
      lobster: apply
  → Lobster ok → MEMORY_WRITE → DONE
```

CTO does NOT run codex, tests, smoke, register, or gateway restart manually. Lobster does everything.

### Non-creation tasks (CTO-driven path)

For factory-codegen and other non-creation work, CTO orchestrates directly and delegates to codex/claude as needed. See `state-machine.md` Path 2.
