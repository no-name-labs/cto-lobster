...
def main():
    ...
    print(json.dumps({
        ...
        "action": "cron_list",
        "count": len(jobs),
        "jobs": [{"id": j.get("id"), "name": j.get("name"), "agent": j.get("agentId"), "schedule": j.get("schedule", {}).get("expr", "") or (f"once @ {j.get('schedule', {}).get('at', '')}" if j.get("schedule", {}).get("kind") == "once" else j.get("schedule", {}).get("kind", "")), "enabled": j.get("enabled", True)} for j in jobs],
    }))
    return 0
